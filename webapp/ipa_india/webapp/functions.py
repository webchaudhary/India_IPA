...
def get_concat_multi_resize(im_list, n_rows, resample=Image.BICUBIC):
    ...
    # Create a new empty image to paste the resized images
    ...
    for i in range(len(im_list_resize)):
            ...
            # If the row is complete, move to the next row
            if (i + 1) % n_cols == 0:
                    pos_x = 0
                    pos_y += im.height
